...
func SecureMiddleware(controllers []server.Controller) mux.MiddlewareFunc {
	return func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			for i := range controllers {
				if fmt.Sprintf("%s%s", server.BaseUrlPrefix, controllers[i].Uri) == req.URL.Path {
			...
			defer cancel()
			jwt := services.NewJwtService(ctx)
